...
def _angle_diff(a: float, b: float) -> float:
    """Та же логика, что в TrackmaniaEnv._angle_diff."""
    return (a - b + math.pi) % (2.0 * math.pi) - math.pi


def augment_state(raw_state: Dict[str, Any], center: CenterLine) -> Dict[str, Any]:
    """
    Добавляет в сырой state поля s, d, tangent_angle, dist_cp, ang_diff
    и приводит yaw из TMInterface к той же системе координат, что и tangent_angle.
    """
    state: Dict[str, Any] = dict(raw_state)  # копия, чтобы не портить оригинал

    pos = state.get("position", (0.0, 0.0, 0.0))
    yaw_raw = float(state.get("yaw", 0.0))   # yaw как отдаёт TMInterface

    # Геометрия относительно центрлайна
    s, d, tangent_angle, dist_cp = center.project_with_extras(pos)

    # Перевод yaw в ту же систему, что и tangent_angle:
    # TMInterface даёт yaw, который смещён на +90° относительно нашей X-оси,
    # поэтому вычитаем π/2 и нормируем в [-π, π).
    yaw_trigo = yaw_raw - math.pi / 2.0
    yaw_trigo = ((yaw_trigo + math.pi) % (2.0 * math.pi)) - math.pi

    # Угол машины относительно направления трассы
    ang_diff = _angle_diff(yaw_trigo, tangent_angle)

    # Сохраняем всё в state (чтобы было видно в логах и в rewards)
    state["s"] = float(s)
    state["d"] = float(d)
...
def main() -> None:
    ...
    try:
        while True:
            # Ждём следующий тик физики от TMInterface
            if not client.wait_for_state(timeout=1.0):
                # Например, если игра в меню — просто ждём дальше
                continue

            raw = client.get_state()
            cur_state = augment_state(raw, center)
