...
class TMIClient:
    ...
    def _send_packet(self, message: Dict[str, Any]) -> None:
    ...
    def _drain_buffer(self) -> None:
        while True:
            ...
            try:
                ...
                message = self._decode_kv_message(line)
                if not message:
                    print(f"[TMIClient] Failed to parse message: {line!r}")
                    continue
    ...
    def _send_neutral_inputs(self) -> None:
        self._send_packet({"type": "inputs", "steer": 0.0, "gas": 0.0, "brake": False})
    ...
    def _decode_kv_message(self, line: str) -> Dict[str, Any]:
        message: Dict[str, Any] = {}
        for token in line.split(";"):
            if not token or "=" not in token:
                continue
            key, raw_value = token.split("=", 1)
            message[key] = self._parse_value(raw_value)
        return message

    def _parse_value(self, raw: str) -> Any:
        if "," in raw:
            return [self._parse_scalar(part) for part in raw.split(",")]
